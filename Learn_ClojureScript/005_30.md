...
そう、このAPIに必要なのは7行のコードだけなのです。`do-request!`ヘルパーIDを書いた苦労が報われました。実際、このコードをもう少しリファクタリングすることができます。というのも、レスポンスのコールバックは、`creat-note!`のコールバックと多くのロジックを共有しているからです。実際、唯一の違いはイベントを発行する関数にあります。エラー処理のロジックを担当する別のヘルパー関数を作ってみましょう。
...
次に、このAPI関数を呼び出すコマンドをディスパッチャに追加します。
...
次のピースは、APIが発する`:notes/received`イベントのハンドラです。APIのレスポンスを正規化するにはかなりの作業が必要ですが、良いことに、その作業はこの章の最初に行っており、あとは簡単です。
...
```
notes/event_handlers/api_data.cljs

それでは、sidebarファイルを開き、APIから送られてきたノートをリストアップするコンポーネントを追加してみましょう。
...
1. ノートを新しいものから順に並べる関数
2. ノートリストを生データに対するリアクションとして定義する
3. コンポーネントがマウントされたときにノートを要求する

ノートリストでは、最新のノートを最初に表示したいと考えていますが、アプリケーションの状態ではマップ内のノートしかなく、順序は定義されていません。並べ替えられたリストを取得するには、基礎となるノートデータが変更されたときにのみ再計算されるリアクションを作成します。スプレッドシートのセルの例えを思い出してください。リアクションは、セルをつなぐ数式のようなものです。ここで足りないのは`link`コンポーネントですので、それを追加しましょう。
...
linkコンポーネントは、ボタンと同様の動作をしますが、現在のルートがリンクのターゲットと一致したときに、アクティブクラスを追加します。これにより、現在のリンクが強調表示される典型的なナビゲーションバーの機能を実現することができます。このコンポーネントは `routes` 名前空間の `get-url` と `matches?` の2つの新しい関数に依存しているので、それらを追加しましょう。
...
`get-url`関数は、ルートパラメータからURL文字列を生成します。これは、新しいURLに移動してルートを推測する必要がある場合とは、まったく逆の動作です。`matches?`関数は、2つのルートパラメータを比較して、同じURLを生成するかどうかをテストします。この関数は、ブラウザが現在、あるリンクのターゲットにいるかどうかを判断するために使用されます。

##### 既存のノートの編集

新しいノートを作成するためのフォームはすでにあるので、今度はそれを少し一般化して、新しいノートの作成と既存のノートの編集の両方に対応できるようにする必要があります。ここでは、ユーザーがフォームに移動したときに、フォームデータを空の状態にするか、ノートを読み込むかを決定するルーティングロジックにフックする方法をとります。フォームの中では、いくつかのラベルを作成状態か編集状態かで条件付けし、作成時と更新時では異なるアクションを実行します。同じビューを再利用しているので、メインコンポーネントのビュースイッチャーに別のエントリを追加しましょう。
...
次に、APIとコマンドディスパッチャに必要なパーツを追加していきましょう。まず、APIには2つの関数が必要です。1つは更新を実行するため、もう1つは1つのノートを取得するためです。更新エンドポイントは更新されたノートを返さないので、コピーが最新であることを確認するために、更新に続いてフェッチを行います。
...
ノートを更新したときには、アプリケーションの状態にすぐに反映させたいので、`:note/updated`イベント用のイベントハンドラを追加する必要があります。このハンドラでは、新しく更新されたノートを全体的に取得するアクションもディスパッチします。また、レスポンスが戻ってきたときに、このノートをステートにマージするハンドラを追加します。
...
1. 更新時にノートを再取得
2. 大量の `:notes/received` イベントに使用したのと同じマージロジックを再利用します。

この機能に必要な状態管理の最後の部分は、ルーティングイベントハンドラへのフックです。
...
1. あるノートの編集ビューへのルートが与えられた場合、そのノートをステートから返す
2. 常に現在のルートを更新する
3. 作成ルートに移動するときは、フォームの状態をクリアする
4. 編集ルートに移動するときは、初期のフォーム状態として対応するノートを複製する

以前、このハンドラはステートの `:current-route` を更新するだけでしたが、ルートに応じて実行される条件付きの更新を追加しただけです。

では、ノートフォームに戻って、適切なラベルが表示され、ユーザーがノートを作成しているか編集しているかに応じて、適切な保存アクションがディスパッチで使用されるように更新してみましょう。新しいノートかどうかを判断するために、フォームデータにIDがあるかどうかをチェックする`is-new?`ヘルパーを導入します。
...
```
notes/ui/views/note_form.cljs

1. 条件に応じて2つのシンボルを同時にバインドする

比較的小さな変更で、ノートの編集ができるようになりました。

**課題**

`:notes/create` と `:notes/update` のどちらかにディスパッチするのではなく、一般的な `:notes/save` コマンドを使用して、ノートが新しいかどうかに応じて異なる API エンドポイントを呼び出すようにしてみましょう。

##### ノートのタグ付け

ノートアプリでサポートする最後の機能は、ノートにタグを適用したり、システムに新しいタグを追加したりする機能です。サポートする必要のあるAPIエンドポイントは3つあります。

- すべてのタグの一覧表示
- 新しいタグの作成
- ノートへのタグ付け

それぞれのアクションにコマンドを追加し、次に必要なAPI関数、そしてイベントハンドラを追加していきましょう。
...
3つのコマンドと3つのAPI関数を追加したので、次は3つのイベントハンドラを追加するのは当然のことです。
...
タグのリストを受け取ると、レッスンの最初に書いた`make-index`関数を使ってidでインデックスを作成し、すでにstateにあるタグの上にマージします。タグを作成するときは、インデックス化されたタグに追加するだけです。最後に、ノートにタグを付けると、「notes-by-tag」と「tags-by-notes」の両方のインデックスにエントリを追加します。

最後に追加しなければならないのは、タグを管理するためのUIです。これは、ノートの作成/編集フォームの一部として追加します。
...
ここでの主な変更点は、これから作成する`tag-selector`コンポーネントの使用です。ここでは、ノートフォームとタグセレクターがページ上で隣り合わせになるように、入れ子のレベルを追加するためにDOMの一部を再構築しました。できるだけシンプルにするために、保存されているノートへのタグの追加のみをサポートします。そうしないと、新しいノートに追加したいノートを記録しておき、作成されたノートのIDを知ってから追加しなければなりません。

以下は、tag-selectorコンポーネント全体と、その依存関係のリストです。ここにはたくさんのことが書かれているので、じっくりと理解してください。このファイルの大部分は、タグ、インデックス、編集中のノートの間でデータを結合するリアクションの作成に費やされています。
...
この `tag-selector` コンポーネントは、ユーザが現在閲覧しているノートに適用されたすべてのタグのコレクションを表示します。また、残りのラベルが表示されている引き出しがあり、これをクリックすることで、ラベルを適用することができます。最後に、ユーザーはテキストボックスに新しいラベルの名前を入力してEnterキーを押すと、新しいラベルが作成されます。

まとめ

さて、このキャップストーンプロジェクトに最後の機能を追加したところで、自分を褒めてあげましょう。あなたはこのキャップストーンを完成させただけでなく、ClojureScriptを学ぶこの旅をやり遂げたのです。この本では、シンタックスの基本的なレッスンから始めました。その後、基本的な概念をより有用なパターンや構造に統合するプロジェクトに進みました。最後に、この最後のキャップストーンでは、しっかりと構造化された、拡張可能なUIアプリケーションを作成しました。このアプリケーションは、関数型プログラミング、宣言型UI、不変の状態管理を取り入れた意図的なアーキテクチャを持っており、おもちゃのようなプロジェクトではありません。

ClojureScriptプログラマーの皆さん、この奇妙で素晴らしい言語で、基礎から実際のプログラミングまでの旅にお付き合いいただき、ありがとうございました。さあ、素晴らしいものを作ってください。
