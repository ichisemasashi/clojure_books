...
このAPI名前空間では、非常にシンプルなメッセージング・プロトコルを定義しています。UI と API の両方が、シリアライズされた ClojureScript データ構造であるメッセージを相互に送信します。最初の要素はメッセージ・タイプを識別するキーワードで、2 番目の要素は任意のタイプのオプションのペイロードになります。ここでは、標準ライブラリの`pr-str`および`read-string`関数を使用して、ClojureネイティブのEDN形式1を使用してデータ構造をシリアライズ／デシリアライズします。
