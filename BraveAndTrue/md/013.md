...
You can attach this function to `fred` with `add-watch.` The general form of `add-watch` is `(add-watch `[ref key watch-fn]{.LiteralItal}`)`.  In this example, we're resetting `fred`'s state, adding the `shuffle-alert` watch function, and then updating `fred`'s state a couple of times to trigger `shuffle-alert`:
